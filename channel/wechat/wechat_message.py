...
class WechatMessage(ChatMessage):
    ...
    def download_file(self, itchat_msg, path):
        try:
            if not hasattr(itchat_msg, 'download'):
                raise AttributeError("itchat_msg object has no attribute 'download'")
            logger.info(f"开始下载文件: {path}")
            itchat_msg.download(path)
            logger.info(f"文件成功下载到: {path}")
