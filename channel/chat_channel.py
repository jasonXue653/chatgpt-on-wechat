...
try:
    from voice.audio_convert import any_to_wav,any_to_png
...
class ChatChannel(Channel):
    ...
    def _generate_reply(self, context: Context, reply: Reply = Reply()) -> Reply:
        ...
        if not e_context.is_pass():
            ...
            if context.type == ContextType.TEXT or context.type == ContextType.IMAGE_CREATE:  # 文字和图片消息
                ...
                try:
                    any_to_png(file_path, png_path)
                    print("图片保存成功")
                except Exception as e:  # 转换失败，直接使用原始文件路径
                    logger.warning("[chat_channel] any to png error, use raw path. " + str(e))
                    png_path = file_path
    ...
    def _decorate_reply(self, context: Context, reply: Reply) -> List[Reply]:
        replies = []
            ...
            if not e_context.is_pass() and reply and reply.type:
                ...
                if reply.type == ReplyType.TEXT:
                    ...
                    if desire_rtype == ReplyType.VOICE and ReplyType.VOICE not in self.NOT_SUPPORT_REPLYTYPE:
                        ...
                        voice_reply = self._decorate_reply(context, voice_reply)[0]
                        replies.append(voice_reply)
                    if context.get("isgroup", False):
                ...
                elif reply.type in [ReplyType.IMAGE_URL, ReplyType.VOICE, ReplyType.IMAGE, ReplyType.FILE,
                                    ReplyType.VIDEO, ReplyType.VIDEO_URL]:
                    pass
                else:
    ...
    def _send_reply(self, context: Context, replies: List[Reply]):
